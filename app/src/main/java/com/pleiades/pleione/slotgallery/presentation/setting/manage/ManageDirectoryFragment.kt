...
class ManageDirectoryFragment : Fragment() {
        ...
        addResultLauncher = registerForActivityResult(StartActivityForResult()) { result: ActivityResult ->
            if (result.data != null) {
                result.data!!.data.also { uri ->
                    ...
                    // case not duplicated
                    if (!selectedSlot.directoryOverviewMutableList.contains(directoryOverview)) {
                        ...
                        // add directory
                        selectedSlot.directoryOverviewMutableList.add(directoryOverview)

                        // notify item inserted
                        recyclerAdapter.notifyItemInserted(selectedSlot.directoryOverviewMutableList.size - 1)
    ...
    inner class ManageDirectoryRecyclerAdapter : RecyclerView.Adapter<ManageDirectoryRecyclerAdapter.ManageDirectoryViewHolder>() {
            ...
            init {
                ...
                removeButton.setOnClickListener {
                    ...
                    if (position < COUNT_DEFAULT_DIRECTORY) {
                        selectedSlot.directoryOverviewMutableList[position] = selectedSlot.directoryOverviewMutableList[position].copy(
                            isVisible = !isVisible
                        )
                        notifyItemChanged(position)
                    } else {
                        selectedSlot.directoryOverviewMutableList.removeAt(position)
        ...
        override fun onBindViewHolder(holder: ManageDirectoryViewHolder, position: Int) {
            // case title
            holder.titleEditText.setText(selectedSlot.directoryOverviewMutableList[position].lastPath)

            // case remove button
            if (position < COUNT_DEFAULT_DIRECTORY) {
                if (selectedSlot.directoryOverviewMutableList[position].isVisible)
        ...
        override fun getItemCount(): Int {
            return selectedSlot.directoryOverviewMutableList.size
        }
    }
}